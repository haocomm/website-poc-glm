const require_options = require('./options-CdNcHcC6.cjs');
let unplugin = require("unplugin");
unplugin = require_options.__toESM(unplugin);
let node_fs = require("node:fs");
node_fs = require_options.__toESM(node_fs);
let pathe = require("pathe");
pathe = require_options.__toESM(pathe);
let __vue_macros_common = require("@vue-macros/common");
__vue_macros_common = require_options.__toESM(__vue_macros_common);
let tinyglobby = require("tinyglobby");
tinyglobby = require_options.__toESM(tinyglobby);
let __vue_compiler_sfc = require("@vue/compiler-sfc");
__vue_compiler_sfc = require_options.__toESM(__vue_compiler_sfc);
let json5 = require("json5");
json5 = require_options.__toESM(json5);
let yaml = require("yaml");
yaml = require_options.__toESM(yaml);
let chokidar = require("chokidar");
chokidar = require_options.__toESM(chokidar);
let picomatch = require("picomatch");
picomatch = require_options.__toESM(picomatch);
let __babel_generator = require("@babel/generator");
__babel_generator = require_options.__toESM(__babel_generator);
let ast_walker_scope = require("ast-walker-scope");
ast_walker_scope = require_options.__toESM(ast_walker_scope);
let mlly = require("mlly");
mlly = require_options.__toESM(mlly);
let local_pkg = require("local-pkg");
local_pkg = require_options.__toESM(local_pkg);
let unplugin_utils = require("unplugin-utils");
unplugin_utils = require_options.__toESM(unplugin_utils);
let magic_string = require("magic-string");
magic_string = require_options.__toESM(magic_string);
...
var TreeNodeValueParam = class extends _TreeNodeValueBase {
	...
	get re() {
		return this.subSegments.filter(Boolean).map((segment) => {
			if (typeof segment === "string") return escapeRegex(segment);
			if (segment.isSplat) return "(.*)";
			let re = segment.repeatable ? "(.+?)" : "([^/]+?)";
			if (segment.optional) re += "?";
			return re;
		}).join("");
	}
	toString() {
...
${state.matchableRecords.sort((a, b) => compareRouteScore(a.score, b.score)).map(({ varName, path: path$1 }) => `  ${varName},  ${" ".repeat(String(state.id).length - varName.length + 2)}// ${path$1}`).join("\n")}
])
`;
}
/**
...
function generateRouteRecord$1({ node, parentVar, state, options, importsMap, paramParsersMap }) {
	...
	if (!shouldSkipNode) {
		...
		if (isMatchable) {
			...
			recordComponents = generateRouteRecordComponent(node, "  ", options.importMode, importsMap);
		} else {
...
*/
function generateRouteRecordPath({ node, importsMap, paramParsersMap }) {
	if (!node.isMatchable()) return "";
